...
$CONFIG = '{"lang":"ja","error_reporting":false,"show_hidden":false,"hide_Cols":false,"calc_folder":false,"theme":"light"}';

use Illuminate\Support\Facades\Log;

/**
 * File Browsing V0.0.1
 * aizenedc@gmail.com
...
$default_timezone = 'Asia/Tokyo'; // Tokyo

// Root path for file manager
// use absolute path of directory i.e: '/var/www/folder' or $_SERVER['DOCUMENT_ROOT'].'/folder'
// 現在実行されているスクリプトの絶対パス /var/www/html/public/index.php
// $root_path = $_SERVER['SCRIPT_FILENAME'];

// 現在実行されているスクリプトが存在するドキュメントルートディレクトリ /var/www/html/public
$root_path = $_SERVER['DOCUMENT_ROOT'];
// echo $root_path;

// Server hostname. Can set manually if wrong
$http_host = $_SERVER['HTTP_HOST'];     // localhost:8573  133.18.169.27

// Root url for links in file manager.Relative to $http_host. Variants: '', 'path/to/subfolder'
// Will not working if $root_path will be outside of server document root
$root_url = '';
// $root_url = $http_host.'/public/actdata/userdata';
// echo $root_url;
...
// DOCUMENT_ROOTの取得 /var/www/html/public
$config_file = $_SERVER['DOCUMENT_ROOT'].'/config/filemng_config.php';
...
//$file = $_SERVER["PHP_SELF"]; // [/index.php/filemng]
//$fm_url = substr($file, 10, 8);  // 10番目から8文字 [/index.php/filemng]->[/filemng]
//  var_dump($fm_url);
define('FM_SELF_URL', ($is_https ? 'https' : 'http') . '://' . $http_host . '/filemng');
...
// $root_path = $root_path .'/userdata';
// 2021/11/20
$folderpath      = 'app'. '/' . 'userdata';
$root_path = storage_path($folderpath) ;

//   echo "<h4>root_path1:{{ $root_path }}</h4>";   ///var/www/html/public/userdata

if (!is_dir($root_path)) {

    echo "<h1>root_path2:{{ $root_path }} lang('not found!')</h1>";
    exit;
}
...
// SuperUser 全部 それ以外はDownloadだけ
if($admin_flg == 3) {
    define('FM_READONLY', false);        // falseで□
} else {
    define('FM_READONLY', true);        // falseで□
}
...
if (isset($_POST['ajax']) && !FM_READONLY) {
    ...
    if (isset($_POST['type']) && $_POST['type'] == "save") {
        ...
        // }
        // 苦労した!!!
        // $fullPath = FM_ROOT_PATH . '/'; 'app'. '/' . 'userdata'. '/' .
        $folder = $customers->foldername;
        // 2021/11/20 Public->Strage
        // $path = FM_ROOT_PATH.'/'. $folder;
        $folderpath      = 'app'. '/' . 'userdata'. '/' . $folder;
...
define('FM_CUSTOMER_NAME', $customername);

// // get current path
$path = FM_ROOT_PATH.'/'. $folder;
//  echo "<h4>path2:{{ $path }}</h4>";   ///var/www/html/public/userdata
// 以下コメントにした
// if (FM_PATH != '') {
//     $path .= '/' . FM_PATH;
// }
...
//$path = '/var/www/html/public/userdata';
...
// echo "<h4>parent3:{{ $parent }}</h4>";  //null

$objects = is_readable($path) ? scandir($path) : array();
$folders = array();
$files = array();
$current_path = array_slice(explode("/",$path), -1)[0];
// echo "<h4>current_path4:{{ $current_path }}</h4>";  //userdata
// Log::debug('filemng.post current_path $file = ' . print_r($current_path, true));
if (is_array($objects) && fm_is_exclude_items($current_path)) {
    foreach ($objects as $file) {
        if ($file == '.' || $file == '..') {
            continue;
        }
        // 2022/11/03 隠しファイルを表示する
            ...
            <div class="card-body">
                <div class="row">
                    <div class="col-xs-12 col-sm-6">
                        <p><h3><a href="https://www.aizen-sol.co.jp" target="_blank" class="app-v-title"> <?php echo APP_TITLE; ?> <?php echo VERSION; ?></a></h3></p>
                        <p>Author: Prasath Mani</p>
                        <p>Mail Us: <a href="mailto:aizenedc@gmail.com">aizenedc[at]gmail.com</a> </p>
                    </div>
                    <div class="col-xs-12 col-sm-6">
                        <div class="card">
                            <ul class="list-group list-group-flush">
                                <li class="list-group-item"><a href="https://www.aizen-sol.co.jp" target="_blank"><i class="fa fa-question-circle"></i> <?php echo lng('Help Documents') ?> </a> </li>
                                <li class="list-group-item"><a href="https://www.aizen-sol.co.jp" target="_blank"><i class="fa fa-bug"></i> <?php echo lng('Report Issue') ?></a></li>
...
if (isset($_GET['view'])) {
    ...
    $file = str_replace('/', '', $file);
    // 2022/11/21 ERROR: Undefined array key "exclude_items"の対応
            ...
            <?php if(!$quickView) { ?>
                ...
                <p class="break-word">
                    ...
                    if (($is_zip || $is_gzip) && $filenames !== false) {
                        ...
                        {{-- 2022/12/09 --}}
                        {{-- Division by zero (View: /actver/resources/views/filemng/post.blade.php:1687) --}}
                        {{-- Compression: <?php echo round(($total_comp / $total_uncomp) * 100) ?>%<br> --}}
                        @php
                            if($total_comp != 0) {
                                $round_comp = ($total_comp / $total_uncomp) * 100;
                            } else {
                                $round_comp = 0;
                            }
                        @endphp
                        Compression: <?php echo round($round_comp) ?>%<br>
                        <?php
                    }
                    // Image info
                    if ($is_image) {
        ...
        <div id="card_newsrepo_list_edit" class="card card-primary">
            ...
            <div class="col-8">
                <a class="navbar-brand" >
                    <input type="hidden"  class="hidden" name="id" value="{{ old('id', $uploadusers->id) }}">
                    @php
                        $textarea = "";
                        if (isset($uploadusers->txt_memo)) {
                            $textarea = $uploadusers->txt_memo;
                            $textarea = htmlspecialchars($textarea);
                        }
                    @endphp
                    <!-- ３行にしたいテキストエリア -->
                    <textarea class="row-3" id="txt_memo" name="txt_memo" >{{$textarea}}</textarea>
                    <button style="display:inline-block; margin-bottom:5px;" type="submit" class="btn btn-sm btn-primary">保存</button>
                </a>
            </div>
            <style>
                /** ３行ピッタシに調整 */
...
<a title="<?php echo lng('Delete') ?>" href="?p=<?php echo urlencode(FM_PATH) ?>&amp;del=<?php echo urlencode($f) ?>" onclick="return confirm('<?php echo lng('Delete').' '.lng('File').'?'; ?>\n \n ( <?php echo ($f) ?> )');"> <i class="fa fa-trash-o"></i></a>
    ...
    <div class="row">
        ...
        <div class="col-3 d-none d-sm-block"><a href="https://www.aizen-sol.co.jp" target="_blank" class="float-right text-muted"><?php echo APP_TITLE; ?> <?php echo VERSION; ?></a></div>
        <?php else: ?>
            <div class="col-12"><a href="https://www.aizen-sol.co.jp" target="_blank" class="float-right text-muted"><?php echo APP_TITLE; ?> <?php echo VERSION; ?></a></div>
        <?php endif; ?>
    </div>
...
// Log::debug('filemng.post fm_is_exclude_items $file = ' . print_r($file, true));
    ...
    if (!in_array($file, $exclude_items) && !in_array("*.$ext", $exclude_items)) {
        // Log::debug('filemng.post return true $exclude_items = ' . print_r($exclude_items, true));
        return true;
    }
    // Log::debug('filemng.post return false $exclude_items = ' . print_r($exclude_items, true));
    return false;
}
...
function fm_get_file_mimes1012($extension)
{
    ...
    $fileTypes['config'] = ['text/plain'];  //add
    $fileTypes['json'] = ['text/plain'];    //add
...
class FM_Config
{
    ...
    function __construct()
    {
        ...
        $file = $root_url.$_SERVER["PHP_SELF"]; // [/index.php/filemng]
        $fm_url = substr($file, 10, 8);  // 10番目から8文字 [/index.php/filemng]->[/filemng]
        // echo var_dump($fm_url);
        ...
        if (strlen($CONFIG)) {
            ...
            $msg = 'AccountingCloud<br>Error: Cannot load configuration';
            if (substr($fm_url, -1) == '/') {
                $fm_url = rtrim($fm_url, '/');
                $msg .= '<br>';
...
function fm_show_nav_path($path)
{
    ...
    <nav class="navbar navbar-expand-lg <?php echo $getTheme; ?> mb-4 main-nav <?php echo $isStickyNavBar ?>">
        ...
        <a class="navbar-brand" href=""> <?php echo FM_CUSTOMER_NAME ?> </a>
        <button class="navbar-toggler" type="button" data-toggle="collapse" data-target="#navbarSupportedContent" aria-controls="navbarSupportedContent" aria-expanded="false" aria-label="Toggle navigation">
            <span class="navbar-toggler-icon"></span>
        </button>
        <div class="collapse navbar-collapse" id="navbarSupportedContent">
<!-- $foldername -->
{{-- <a title="<?php echo urlencode($foldername) ?>&amp;settings=1"></a> --}}
            <!-- ホームアイコン-->
            <!-- homeicon change fa fa-home --fa fa-bank -->
            <?php
            $path = fm_clean_path($path);
            $root_url = "<a href='?p='><i class='fa fa-bank' aria-hidden='true' title='" . FM_ROOT_PATH . "'></i></a>";
...
<a style="color: rgb(0, 0, 255); " title="<?php echo '一括ダウンロード' ?>" class="dropdown-item nav-link" href="{{route('filemngdown')}}"><i class="fa fa-download" aria-hidden="true"></i> <?php echo '一括ダウンロード' ?></a>
                    ...
                    <?php if (FM_USE_AUTH): ?>
                    @csrf   <!-- koko -->
                    <!-- usericon change fa fa-user-circle-o --fa fa-address-card -->
                    <li class="nav-item avatar dropdown">
                        {{-- 2022/10/20 Auth::user();コメント--}}
                        {{-- <a class="nav-link dropdown-toggle" id="navbarDropdownMenuLink-5" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"> <i class="fa fa-address-card" style="color: rgba(0, 140, 255, 1.0); "><?php $user = Auth::user(); ?> {{ $user->name }}</i></a> --}}
<a style="color: rgb(255, 0, 0); " title="<?php echo '一覧に戻る' ?>" class="dropdown-item nav-link" href="{{route('uploaduser')}}"><i class="fa fa-reply" aria-hidden="true"></i> <?php echo '一覧に戻る' ?></a>
                        ...
                        <div class="dropdown-menu dropdown-menu-right <?php echo fm_get_theme(); ?>" aria-labelledby="navbarDropdownMenuLink-5">
                            ...
                            {{-- <a title="<?php echo lng('Back') ?>" class="dropdown-item nav-link" href="{{route('uploaduser')}}"><i class="fa fa-reply" aria-hidden="true"></i> <?php echo lng('Back') ?></a> --}}
                    {{-- 2022/10/20 --}}
{{-- <a style="color: rgb(255, 0, 0); " title="<?php echo '一覧に戻る' ?>" class="dropdown-item nav-link" href="{{route('uploaduser')}}"><i class="fa fa-reply" aria-hidden="true"></i> <?php echo '一覧に戻る' ?></a> --}}
                            <form id="logout-form" action="{{ route('logout') }}" method="POST" style="display: none;">
                                @csrf
                                <a title="<?php echo lng('Logout') ?>" class="dropdown-item nav-link" href="{{route('logout')}}" ><i class="fa fa-sign-out" aria-hidden="true"></i> <?php echo lng('Logout') ?></a>
                            </form>
                        </div>
                    </li>
                    <?php else: ?>
...
<head>
    ...
    <meta name="description" content="Web based File Browsing in PHP, Manage your files efficiently and easily with Actver File Browsing">
    <meta name="author" content="Aizen Solution">
...
<div id="wrapper" class="container-fluid">
    ...
    <!-- 2023/08/25 F12検証 use of <label for=FORM_ELEMENT> 以下コメント -->
    <!-- New Item creation Incorrect -->
...
<script>
    ...
    function fm_search() {
        ...
        } else { searchWrapper.html("PS: 最低 3 文字必要です!"); }
    }
    ...
    // 2023/08/21 F12検証でundefined (setting 'ntf')の対応で以下コメント
    // Dom Ready Event
...
function lng($txt) {
    ...
    // English Language
    $tr['en']['AppName']        = 'AccountingCloud';   		$tr['en']['AppTitle']           = 'AccountingCloud';
    $tr['en']['Login']          = 'ログイン';               $tr['en']['Username']           = 'Username';
    $tr['en']['Password']       = 'パスワード';             $tr['en']['Logout']             = 'ログアウト';
    $tr['en']['Move']           = '移動';                   $tr['en']['Copy']               = '複写';
    $tr['en']['Save']           = '保存';                   $tr['en']['SelectAll']          = '全選択';
    $tr['en']['UnSelectAll']    = '全解除';                 $tr['en']['File']               = 'ファイル';
    $tr['en']['Back']           = '戻る';                   $tr['en']['Size']               = 'サイズ';
    $tr['en']['Perms']          = '権限';                   $tr['en']['Modified']           = '更新日時';
    $tr['en']['Owner']          = '所有者';                 $tr['en']['Search']             = '検索';
    $tr['en']['NewItem']        = '新規作成';               $tr['en']['Folder']             = 'フォルダ';
    $tr['en']['Delete']         = '削除';                   $tr['en']['Rename']             = 'リネーム';
    $tr['en']['CopyTo']         = '宛先を指定してコピー';   $tr['en']['DirectLink']         = '直接リンク';
    $tr['en']['UploadingFiles'] = 'ファイルをアップロード'; $tr['en']['ChangePermissions']  = '権限を変更';
    $tr['en']['Copying']        = 'ファイルをコピー';       $tr['en']['CreateNewItem']      = '新規作成';
    $tr['en']['Name']           = '名前';                   $tr['en']['AdvancedEditor']     = '拡張エディタで編集';
    $tr['en']['RememberMe']     = 'Remember Me';            $tr['en']['Actions']            = '操作';
    $tr['en']['Upload']         = 'アップロード';           $tr['en']['Cancel']             = 'キャンセル';
    $tr['en']['InvertSelection']= '選択の切り替え';         $tr['en']['DestinationFolder']  = '宛先フォルダ';
    $tr['en']['ItemType']       = '種類';                   $tr['en']['ItemName']           = '名前';
    $tr['en']['CreateNow']      = '作成する';               $tr['en']['Download']           = 'ダウンロード';
    $tr['en']['Open']           = '開く';                   $tr['en']['UnZip']              = '解凍';
    $tr['en']['UnZipToFolder']  = 'フォルダに解凍';         $tr['en']['Edit']               = '編集';
    $tr['en']['NormalEditor']   = '通常エディタで編集';     $tr['en']['BackUp']             = 'バックアップ';
    $tr['en']['SourceFolder']   = '元フォルダ';             $tr['en']['Files']              = 'ファイル';
    $tr['en']['Move']           = '移動';                   $tr['en']['Change']             = '変更';
    $tr['en']['Settings']       = '設定';                   $tr['en']['Language']           = '言語';
    $tr['en']['Folder is empty']  = '空のフォルダです';     $tr['en']['PartitionSize']      = '空き容量';
    $tr['en']['ErrorReporting'] = 'エラーを表示';           $tr['en']['ShowHiddenFiles']    = '隠しファイルを表示';
    $tr['en']['Full size']      = '合計サイズ';             $tr['en']['Help']               = 'ヘルプ';
    $tr['en']['Free of']        = '容量';                   $tr['en']['Preview']            = 'プレビュー';
    $tr['en']['Help Documents'] = 'ヘルプドキュメント';     $tr['en']['Report Issue']       = '問題を報告';
    $tr['en']['Generate']       = '生成';                   $tr['en']['FullSize']           = '合計サイズ';
    $tr['en']['FreeOf']         = '容量';                   $tr['en']['CalculateFolderSize']= 'フォルダサイズを計算';

    $tr['en']['ProcessID']      = 'Process ID';             $tr['en']['Created']    = '新規作成';
    ...
    //2022/11/08 strlen(): Passing null to parameter #1 ($string) of type string is deprecated 4223
