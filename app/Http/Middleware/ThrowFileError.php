...
class ThrowFileError
{
    public function handle(Request $request, Closure $next)
    {
        ...
        foreach ($request->allFiles() as $file) {
            ...
            // 2025/01/11 $file->getError() Comment
            // ERROR: Call to a member function getError() on array {"exception":"[object] (Error(code: 0): Call to a member function getError()
            //     on array at /var/www/html/actver/app/Http/Middleware/ThrowFileError.php:24
            // Log::debug('ThrowFileError !== UPLOAD_ERR_OK');     // 2025/01/11
            // Log::debug('ThrowFileError $file->getError() = ' . print_r($file->getError(), true));
        ...
        if (! empty($errors)) {
            Log::debug('ThrowFileError errors = HTTP_BAD_REQUEST');     // 2025/01/11
